...
import {  saveChange, handleChangeValue, clickEdit, validateReg, clearState } from '../../store/dataSlice';
...
const UserAccount = () => {
    ...
    const onClick = () => {
        dispatch(clickedEdit ? saveChange() : clickEdit())
    }
    ...
    return (<>
        <section className={style.enterForm}>
            ...
            <div className={style.user__wrapper}>
                <Text parent={'user__form__title'}
                    text={`Привет, ${valueUser.name}`}
                    textType={'h2'} />
                {clickedEdit ?
                    <Form parent={'enter__form'}>
                        <Input onChange={(e) => dispatch(handleChangeValue({ e }))}
                            inputname={'name'}
                            parent={'enter__form__user'}
                            text={'Имя'} />
                        <Input onChange={(e) => dispatch(handleChangeValue({ e }))}
                            inputname={'email'}
                            parent={'enter__form__user'}
            ...
            <div className={style.button__wrapper}>
                <Button disabled={saveButtonStatus}
                    onClick={onClick}
                    parent={'button__edit'}
                    text={clickedEdit ? 'Сохранить' : 'Редактировать'} />
                <Link onClick={onExit}
                    className={style.button__exit} >Выйти из аккаунта</Link>
            </div>
        </section>
